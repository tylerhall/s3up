...
<?PHP
    ...
    {
        echo "Usage: {$GLOBALS['argv'][0]} [-ctxz] [options] s3-bucket s3-filename filename\n";
    ...
    elseif(substr($s3_filename, -1, 1) == '/') // Assume $s3_filename is actually a directory, so upload files "into" it instead
    {
        $s3_dir = $s3_filename;
        if($s3_dir == '/') $s3_dir = '';
        $s3_dir = ltrim($s3_dir, '/');
    ...
    // TODO: Can probably refactor this as it contains duplicate code
    function upload_directory($bucket, $s3_dir, $local_path, $local_dir, $args)
    {
        ...
        while(false !== ($file = readdir($dhandle)))
        {
            ...
            $file = "$local_path/$local_dir/$file";
            if(is_dir($file))
            {
